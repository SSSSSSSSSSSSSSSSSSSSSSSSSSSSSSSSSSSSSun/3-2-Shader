...
void Renderer::Initialize(int windowSizeX, int windowSizeY)
{
	...
	m_ParticleTexture = CreatePngTexture("./Texture/fire particle.png", GL_NEAREST);
...
void Renderer::DrawParticle()
{
	...
	int uTextureLoc{ glGetUniformLocation(m_ParticleShader, "u_Texture") };
	...
	int aUVLoc{ glGetAttribLocation(m_ParticleShader, "a_UV") };
	...
	glVertexAttribPointer(aUVLoc, 2, GL_FLOAT, GL_FALSE, sizeof(float) * stride, (GLvoid*)(sizeof(float) * 15));
...
void Renderer::DrawGridMesh()
{
	...
	int uPointLoc{ glGetUniformLocation(shader, "u_Points") };
	glUniform4fv(uPointLoc, 100, m_Points);
...
void Renderer::DrawTexture(float x, float y, float sizeX, float sizeY, GLuint TexID0, GLuint TexID1, GLuint method)
{
...
{
	// x, y, z, value, r, g, b, a, sTime, vx, vy, vz, mass, period, tx, ty
	int nFloatCountPerVertex{ 3 + 1 + 4 + 1 + 3 + 1 + 1 + 1 + 2};
...
void Renderer::CreateFS(float r, float g, float b, float a)
{
...
void Renderer::CreateFBOs()
{
	...
	glGenFramebuffers(2, m_PingpongFBO.data());
	glGenTextures(2, m_PingpongTexture.data());
	for (int i = 0; i < 2; ++i)
	{
